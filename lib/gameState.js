...
async function saveGameState(state, retries = 3) {
  const redis = getRedisClient();
  ...
  if (redis) {
    ...
    for (let attempt = 1; attempt <= retries; attempt++) {
      try {
        ...
        console.log(`[REDIS] Saved game state: Q${state.currentQuestionIndex + 1}, phase=${state.phase} (attempt ${attempt})`);
        
        // Verify the save succeeded by reading it back
        const verifyState = await redis.get(REDIS_KEY);
        if (verifyState && verifyState.currentQuestionIndex === state.currentQuestionIndex) {
          ...
          throw new Error('Save verification failed - state mismatch');
        }
      } catch (error) {
        lastError = error;
        console.error(`[REDIS] Error saving state to Redis (attempt ${attempt}/${retries}):`, error.message);
        if (attempt < retries) {
    ...
    // All retries failed - log error but still update in-memory as fallback
    console.error('[REDIS] Failed to save state after all retries:', lastError);
    inMemoryState = state;
    stateInitialized = true;
    resetGameStateCache();
    return false; // Indicate failure
  } else {
    // No Redis - just update in-memory state
    inMemoryState = state;
    stateInitialized = true;
    resetGameStateCache();
    return true;
...
async function recalculateScores() {
  ...
  // Capture current answered flags and currentAnswers BEFORE doing anything
  const answeredFlags = {};
  const currentAnswersBackup = {};
  Object.keys(state.players).forEach(player => {
    answeredFlags[player] = !!state.players[player].answered;
    // Backup currentAnswers for current question
    if (state.currentAnswers && state.currentAnswers[player]) {
      currentAnswersBackup[player] = { ...state.currentAnswers[player] };
    }
  });
  
  // Reconstruct currentAnswers from allAnswers for current question (in case it was lost)
  const currentQIndex = state.currentQuestionIndex;
  if (!state.currentAnswers) {
    state.currentAnswers = {};
  }
  
  // Ensure currentAnswers includes all answers for current question from allAnswers
  Object.keys(state.players).forEach(player => {
    const playerAnswers = state.allAnswers[player];
    if (playerAnswers && playerAnswers[currentQIndex]) {
      // Use answer from allAnswers if we have it
      state.currentAnswers[player] = { ...playerAnswers[currentQIndex] };
    } else if (currentAnswersBackup[player]) {
      // Fall back to backup if allAnswers doesn't have it yet
      state.currentAnswers[player] = currentAnswersBackup[player];
    }
  });
    ...
    if (answers && typeof answers === 'object') {
      ...
      answerKeys.forEach(qIndexStr => {
        ...
        if (question && typeof question.answerIndex !== 'undefined') {
          ...
          // Update currentAnswers if this is the current question
          if (qIndex === currentQIndex && state.currentAnswers[player]) {
            state.currentAnswers[player].correct = isCorrect;
          }
        }
      });
    }
